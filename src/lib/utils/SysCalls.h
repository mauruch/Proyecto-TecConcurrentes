...
int open(const char *pathname, int flags, int otherFlags) {
...
int fcntl(int fd, int cmd, flock *fl){
	int response = ::fcntl(fd, cmd, fl);
	if (response < 0) {
		throw SysCallException("Error trying to execute a fcntl()");
	}
	return response;
}
