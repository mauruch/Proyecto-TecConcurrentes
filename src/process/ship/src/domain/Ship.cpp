...
Ship::Ship(const unsigned int load, int semId, int shmId) :
		shipload(load), ownSem(semId), shm(shmId), lockShMemDocksSem(
				utils::FILE_FTOK.c_str(), utils::ID_FTOK_LOCK_SHMEM_SEM_DOCKS), controllerQueueFifo(
				utils::CONTROLLER_QUEUE_FIFO), controllerFifo(utils::CONTROLLER_FIFO), craneFifo(utils::CRANE_FIFO), shipFifo(utils::SHIP_FIFO) {
